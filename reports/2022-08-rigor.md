![Rigor Protocol](/static/7e98f53d4533db5f06e8823a26e67639/4e333/Rigor.jpg)
...
*   [High Risk Findings (6)](#high-risk-findings-6)
    ...
    *   [\[H-01\] Untyped data signing](#h-01-untyped-data-signing)
    *   [\[H-02\] Builder can halve the interest paid to a community owner due to arithmetic rounding](#h-02-builder-can-halve-the-interest-paid-to-a-community-owner-due-to-arithmetic-rounding)
    *   [\[H-03\] Builder can call `Community.escrow` again to reduce debt further using same signatures](#h-03-builder-can-call-communityescrow-again-to-reduce-debt-further-using-same-signatures)
    *   [\[H-04\] Project funds can be drained by reusing signatures, in some cases](#h-04-project-funds-can-be-drained-by-reusing-signatures-in-some-cases)
    *   [\[H-05\] Add members to the not yet created community](#h-05-add-members-to-the-not-yet-created-community)
    *   [\[H-06\] Wrong APR can be used when project is unpublished and published again](#h-06-wrong-apr-can-be-used-when-project-is-unpublished-and-published-again-)
*   [Medium Risk Findings (20)](#medium-risk-findings-20)
    
    *   [\[M-01\] `Project.changeOrder()` would work unexpectedly for non SCConfirmed tasks.](#m-01-projectchangeorder-would-work-unexpectedly-for-non-scconfirmed-tasks)
    *   [\[M-02\] Missing upper limit definition in `replaceLenderFee()` of `HomeFi.sol`](#m-02-missing-upper-limit-definition-in-replacelenderfee-of-homefisol)
    *   [\[M-03\] Signature Checks could be passed when `SignatureDecoder.recoverKey()` returns 0](#m-03-signature-checks-could-be-passed-when-signaturedecoderrecoverkey-returns-0)
    *   [\[M-04\] Hash approval not possible when contractor == subcontractor](#m-04-hash-approval-not-possible-when-contractor--subcontractor)
    *   [\[M-05\] Anyone can create disputes if `contractor` is not set](#m-05-anyone-can-create-disputes-if-contractor-is-not-set)
    *   [\[M-06\] Attacker can drain all the projects within minutes, if admin account has been exposed](#m-06-attacker-can-drain-all-the-projects-within-minutes-if-admin-account-has-been-exposed)
    *   [\[M-07\] `Project.raiseDispute()` doesn’t use approvedHashes - meaning users who use contracts can’t raise disputes](#m-07-projectraisedispute-doesnt-use-approvedhashes---meaning-users-who-use-contracts-cant-raise-disputes)
    *   [\[M-08\] Builders must pay more interest when the system is paused.](#m-08-builders-must-pay-more-interest-when-the-system-is-paused)
    *   [\[M-09\] It should not submit a project with no total budget. Requires at least one task with cost > 0](#m-09-it-should-not-submit-a-project-with-no-total-budget-requires-at-least-one-task-with-cost--0)
    *   [\[M-10\] Possible DOS in `lendToProject()` and `toggleLendingNeeded()` function because unbounded loop can run out of gas](#m-10-possible-dos-in-lendtoproject-and-togglelendingneeded-function-because-unbounded-loop-can-run-out-of-gas)
    *   [\[M-11\] Owner of project NFT has no purpose](#m-11-owner-of-project-nft-has-no-purpose)
    *   [\[M-12\] `updateProjectHash` does not check project address](#m-12-updateprojecthash-does-not-check-project-address)
    *   [\[M-13\] In `Project.setComplete()`, the signature can be reused when the first call is reverted for some reason](#m-13-in-projectsetcomplete-the-signature-can-be-reused-when-the-first-call-is-reverted-for-some-reason)
    *   [\[M-14\] Incorrect initialization of smart contracts with Access Control issue](#m-14-incorrect-initialization-of-smart-contracts-with-access-control-issue)
    *   [\[M-15\] `Project.addTasks()` wouldn’t work properly when it’s called from disputes contract.](#m-15-projectaddtasks-wouldnt-work-properly-when-its-called-from-disputes-contract)
    *   [\[M-16\] New subcontractor can be set for a SCConfirmed task without current subcontractor consent](#m-16-new-subcontractor-can-be-set-for-a-scconfirmed-task-without-current-subcontractor-consent)
    *   [\[M-17\] Malicious delegated contractor can block funding tasks or mark tasks as complete](#m-17-malicious-delegated-contractor-can-block-funding-tasks-or-mark-tasks-as-complete)
    *   [\[M-18\] Task Functionality completely sidestepped via `autoWithdraw`](#m-18-task-functionality-completely-sidestepped-via-autowithdraw)
    *   [\[M-19\] `changeOrder` requires subcontractor signature when the subcontractor address is 0](#m-19-changeorder-requires-subcontractor-signature-when-the-subcontractor-address-is-0)
    *   [\[M-20\] `Project.sol` and `Community.sol` have no way to revoke a hash in approvedHashes](#m-20-projectsol-and-communitysol-have-no-way-to-revoke-a-hash-in-approvedhashes)
...
*   [Gas Optimizations](#gas-optimizations)
    ...
    *   [G-01 Cache storage values in memory to minimize SLOADs](#g-01-cache-storage-values-in-memory-to-minimize-sloads)
    *   [G-02 Cache the length of arrays in loops](#g-02-cache-the-length-of-arrays-in-loops)
    *   [G-03 ++i costs less gas compared to i++ or i += 1 in for loops (~5 gas per iteration)](#g-03-i-costs-less-gas-compared-to-i-or-i--1-in-for-loops-5-gas-per-iteration)
    *   [G-04 ++x is more efficient than x++(Saves ~6 gas)](#g-04-x-is-more-efficient-than-xsaves-6-gas)
    *   [G-05 Splitting require() statements that use && saves gas - (saves 8 gas per &&)](#g-05-splitting-require-statements-that-use--saves-gas---saves-8-gas-per-)
    *   [G-06 Comparisons: != is more efficient than > in require (6 gas less)](#g-06-comparisons--is-more-efficient-than--in-require-6-gas-less)
    *   [G-07 Emitting storage values instead of the memory one(saves ~101 gas)](#g-07-emitting-storage-values-instead-of-the-memory-onesaves-101-gas)
    *   [G-08 Using unchecked blocks to save gas](#g-08-using-unchecked-blocks-to-save-gas)
    *   [G-09 Using unchecked blocks to save gas - Increments in for loop can be unchecked ( save 30-40 gas per loop iteration)](#g-09-using-unchecked-blocks-to-save-gas---increments-in-for-loop-can-be-unchecked---save-30-40-gas-per-loop-iteration)
    *   [G-10 Use Custom Errors instead of Revert Strings to save Gas](#g-10-use-custom-errors-instead-of-revert-strings-to-save-gas)
    *   [G-11 x += y costs more gas than x = x + y for state variables](#g-11-x--y-costs-more-gas-than-x--x--y-for-state-variables)
    *   [G-12 Using bools for storage incurs overhead](#g-12-using-bools-for-storage-incurs-overhead)
...
4.  0x52
5.  0xA5DF
6.  [indijanc](https://twitter.com/krenkmet)
7.  [GalloDaSballo](https://twitter.com/gallodasballo)
8.  [wastewa](https://twitter.com/WahWaste)
9.  vlad\_bochok
10.  byndooa
11.  0x1f8b
12.  [hyh](https://twitter.com/0xhyh)
13.  [berndartmueller](https://twitter.com/berndartmueller)
14.  [sseefried](http://seanseefried.org/blog)
15.  cccz
16.  cryptphi
17.  [minhquanym](https://www.linkedin.com/in/minhquanym/)
18.  horsefacts
19.  scaraven
20.  Haipls
21.  TrungOre
22.  [defsec](https://twitter.com/defsec_)
23.  [Deivitto](https://twitter.com/Deivitto)
24.  [0xNazgul](https://twitter.com/0xNazgul)
25.  [MEP](https://twitter.com/meppent)
26.  [Chom](https://chom.dev)
27.  Bahurum
28.  [bin2chen](https://twitter.com/bin2chen)
29.  cryptonue
30.  [c3phas](https://twitter.com/c3ph_)
31.  arcoun
32.  rotcivegaf
33.  simon135
34.  [obront](https://twitter.com/zachobront)
35.  [saneryee](https://medium.com/@saneryee-studio)
36.  [rokinot](twitter.com/rokinot)
37.  ElKu
38.  auditor0517
39.  [smiling\_heretic](https://github.com/SmilingHeretic)
40.  kankodu
41.  [kaden](https://twitter.com/0xKaden)
42.  aez121
43.  neumo
44.  [Dravee](https://twitter.com/BowTiedDravee)
45.  [Ruhum](https://twitter.com/0xruhum)
46.  \_\_141345\_\_
47.  [8olidity](https://twitter.com/8olidity)
48.  [panprog](https://twitter.com/panprog)
49.  codexploder
50.  IllIllI
51.  [CertoraInc](https://twitter.com/CertoraInc) (egjlmn1, [OriDabush](https://twitter.com/ori_dabush), ItayG, and shakedwinder)
52.  [fatherOfBlocks](https://twitter.com/father0fBl0cks)
53.  saian
54.  [Guardian](https://twitter.com/GuardianAudits)
55.  [MiloTruck](https://milotruck.github.io/)
56.  [JC](https://twitter.com/sm4rtcontr4ct)
57.  [benbaessler](https://benbaessler.com)
58.  hake
59.  0xkatana
60.  [gogo](https://www.linkedin.com/in/georgi-nikolaev-georgiev-978253219)
61.  [oyc\_109](https://twitter.com/andyfeili)
62.  [pfapostol](https://t.me/pfahard)
63.  samruna
64.  [joestakey](https://twitter.com/JoeStakey)
65.  ReyAdmirado
66.  [TomJ](https://mobile.twitter.com/tomj_bb)
67.  Rolezn
68.  brgltd
69.  [Extropy](https://twitter.com/extropy)
70.  [Aymen0909](https://github.com/Aymen1001)
71.  [0xSmartContract](https://twitter.com/0xSmartContract)
72.  delfin454000
73.  mics
74.  bobirichman
75.  sikorico
...
82.  Bnke0x0
83.  ajtra
84.  [Tomio](https://twitter.com/meidhiwirara)
85.  [Funen](https://instagram.com/vanensurya)
86.  [Rohan16](https://twitter.com/ROHANJH56009256)
87.  [Sm4rty](https://twitter.com/Sm4rty_)
88.  Waze
89.  sach1r0
90.  [ignacio](https://twitter.com/0xheynacho)
91.  [supernova](https://twitter.com/harshit16024263)
92.  asutorufos
93.  [0xSolus](https://twitter.com/0xSolus)
94.  Noah3o6
95.  [a12jmx](https://twitter.com/a12jmx)
96.  djxploit
97.  [Ch\_301](https://twitter.com/0xch301)
98.  dipp
99.  0xf15ers (remora and twojoy)
100.  bulej93
101.  Soosh
102.  0xNineDec
103.  [exd0tpy](https://github.com/exd0tpy)
104.  ayeslick
105.  poirots ([DavideSilva](https://twitter.com/DavideSilva_), resende, and naps62)
106.  Yiko
107.  0xsolstars ([Varun\_Verma](twitter.com/versatile_crypt) and masterchief)
108.  p\_crypt0
109.  Jujic
110.  Throne6g
111.  NoamYakov
112.  [Chinmay](https://twitter.com/dev_chinmayf)
113.  eierina
114.  jag
115.  [bharg4v](https://twitter.com/Bharg4v)
116.  0x040
117.  ballx
118.  [durianSausage](https://github.com/lyciumlee)
119.  Metatron
120.  [ret2basic](https://twitter.com/ret2basic)
121.  0xsam
122.  [Fitraldys](https://twitter.com/fitraldys)
123.  [gerdusx](https://twitter.com/GerdusM)
124.  kyteg
125.  apostle0x01
126.  lucacez
127.  [dharma09](https://twitter.com/im_Dharma09)
128.  PaludoX0
129.  zeesaw
130.  0xc0ffEE
131.  [tofunmi](https://twitter.com/dediranTofunmi)
132.  [teddav](https://twitter.com/0xteddav)
...
[](#h-01-untyped-data-signing)[\[H-01\] Untyped data signing](https://github.com/code-423n4/2022-08-rigor-findings/issues/75)
-----------------------------------------------------------------------------------------------------------------------------

_Submitted by Lambda, also found by 0x1f8b, 0x52, horsefacts, vlad\_bochok, and wastewa_

[Community.sol#L175](https://github.com/code-423n4/2022-08-rigor/blob/e35f5f61be9ff4b8dc5153e313419ac42964d1fd/contracts/Community.sol#L175)  
[Community.sol#L213](https://github.com/code-423n4/2022-08-rigor/blob/e35f5f61be9ff4b8dc5153e313419ac42964d1fd/contracts/Community.sol#L213)  
[Community.sol#L530](https://github.com/code-423n4/2022-08-rigor/blob/e35f5f61be9ff4b8dc5153e313419ac42964d1fd/contracts/Community.sol#L530)  
[Disputes.sol#L91](https://github.com/code-423n4/2022-08-rigor/blob/f2498c86dbd0e265f82ec76d9ec576442e896a87/contracts/Disputes.sol#L91)  
[Project.sol#L142](https://github.com/code-423n4/2022-08-rigor/blob/f2498c86dbd0e265f82ec76d9ec576442e896a87/contracts/Project.sol#L142)  
[Project.sol#L167](https://github.com/code-423n4/2022-08-rigor/blob/f2498c86dbd0e265f82ec76d9ec576442e896a87/contracts/Project.sol#L167)  
[Project.sol#L235](https://github.com/code-423n4/2022-08-rigor/blob/f2498c86dbd0e265f82ec76d9ec576442e896a87/contracts/Project.sol#L235)  
[Project.sol#L286](https://github.com/code-423n4/2022-08-rigor/blob/f2498c86dbd0e265f82ec76d9ec576442e896a87/contracts/Project.sol#L286)  
[Project.sol#L346](https://github.com/code-423n4/2022-08-rigor/blob/f2498c86dbd0e265f82ec76d9ec576442e896a87/contracts/Project.sol#L346)  
[Project.sol#L402](https://github.com/code-423n4/2022-08-rigor/blob/f2498c86dbd0e265f82ec76d9ec576442e896a87/contracts/Project.sol#L402)  
[Project.sol#L499](https://github.com/code-423n4/2022-08-rigor/blob/f2498c86dbd0e265f82ec76d9ec576442e896a87/contracts/Project.sol#L499)  
...
[](#h-02-builder-can-halve-the-interest-paid-to-a-community-owner-due-to-arithmetic-rounding)[\[H-02\] Builder can halve the interest paid to a community owner due to arithmetic rounding](https://github.com/code-423n4/2022-08-rigor-findings/issues/180)
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

_Submitted by scaraven, also found by 0x52, auditor0517, Deivitto, hansfriese, Lambda, rbserver, simon135, smiling\_heretic, sseefried, and TrungOre_

[Community.sol#L685-L686](https://github.com/code-423n4/2022-08-rigor/blob/5ab7ea84a1516cb726421ef690af5bc41029f88f/contracts/Community.sol#L685-L686)  
...
[](#h-03-builder-can-call-communityescrow-again-to-reduce-debt-further-using-same-signatures)[\[H-03\] Builder can call `Community.escrow` again to reduce debt further using same signatures](https://github.com/code-423n4/2022-08-rigor-findings/issues/161)
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
[Community.sol#L509](https://github.com/code-423n4/2022-08-rigor/blob/5ab7ea84a1516cb726421ef690af5bc41029f88f/contracts/Community.sol#L509)  
...
[](#h-04-project-funds-can-be-drained-by-reusing-signatures-in-some-cases)[\[H-04\] Project funds can be drained by reusing signatures, in some cases](https://github.com/code-423n4/2022-08-rigor-findings/issues/95)
----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
[Project.sol#L386-L490](https://github.com/code-423n4/2022-08-rigor/blob/5ab7ea84a1516cb726421ef690af5bc41029f88f/contracts/Project.sol#L386-L490)  
[Project.sol#L330-L359](https://github.com/code-423n4/2022-08-rigor/blob/5ab7ea84a1516cb726421ef690af5bc41029f88f/contracts/Project.sol#L330-L359)  
[Tasks.sol#L153-L164](https://github.com/code-423n4/2022-08-rigor/blob/5ab7ea84a1516cb726421ef690af5bc41029f88f/contracts/libraries/Tasks.sol#L153-L164)  
...
#### [](#draining-the-project-from-funds)Draining the project from funds
...
#### [](#changing-tasks-costssubcontractor-by-external-users)Changing tasks costs/subcontractor by external users
...
_Note: see warden’s [original submission](https://github.com/code-423n4/2022-08-rigor-findings/issues/95) for full proof of concept._
...
[](#h-05-add-members-to-the-not-yet-created-community)[\[H-05\] Add members to the not yet created community](https://github.com/code-423n4/2022-08-rigor-findings/issues/298)
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

_Submitted by vlad\_bochok, also found by indijanc, Lambda, and wastewa_
...
[](#h-06-wrong-apr-can-be-used-when-project-is-unpublished-and-published-again-)[\[H-06\] Wrong APR can be used when project is unpublished and published again](https://github.com/code-423n4/2022-08-rigor-findings/issues/83)
--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

_Submitted by Lambda, also found by rbserver_

[Community.sol#L267](https://github.com/code-423n4/2022-08-rigor/blob/e35f5f61be9ff4b8dc5153e313419ac42964d1fd/contracts/Community.sol#L267)  
...
[](#m-01-projectchangeorder-would-work-unexpectedly-for-non-scconfirmed-tasks)[\[M-01\] `Project.changeOrder()` would work unexpectedly for non SCConfirmed tasks.](https://github.com/code-423n4/2022-08-rigor-findings/issues/232)
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
[](#m-02-missing-upper-limit-definition-in-replacelenderfee-of-homefisol)[\[M-02\] Missing upper limit definition in `replaceLenderFee()` of `HomeFi.sol`](https://github.com/code-423n4/2022-08-rigor-findings/issues/400)
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

_Submitted by MiloTruck, also found by \_\_141345\_\_, 0x52, 8olidity, cccz, Ch\_301, codexploder, cryptonue, hansfriese, Ruhum, and sseefried_

[Community.sol#L392-L394](https://github.com/code-423n4/2022-08-rigor/blob/main/contracts/Community.sol#L392-L394)  
[HomeFi.sol#L184-L197](https://github.com/code-423n4/2022-08-rigor/blob/main/contracts/HomeFi.sol#L184-L197)  
...
[](#m-03-signature-checks-could-be-passed-when-signaturedecoderrecoverkey-returns-0)[\[M-03\] Signature Checks could be passed when `SignatureDecoder.recoverKey()` returns 0](https://github.com/code-423n4/2022-08-rigor-findings/issues/179)
-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

_Submitted by cryptphi, also found by 0x1f8b and defsec_

[Project.sol#L887](https://github.com/code-423n4/2022-08-rigor/blob/main/contracts/Project.sol#L887)  
[Project.sol#L108-L115](https://github.com/code-423n4/2022-08-rigor/blob/main/contracts/Project.sol#L108-L115)  
...
[](#m-04-hash-approval-not-possible-when-contractor--subcontractor)[\[M-04\] Hash approval not possible when contractor == subcontractor](https://github.com/code-423n4/2022-08-rigor-findings/issues/86)
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

_Submitted by Lambda, also found by hansfriese_

[Project.sol#L859](https://github.com/code-423n4/2022-08-rigor/blob/f2498c86dbd0e265f82ec76d9ec576442e896a87/contracts/Project.sol#L859)  
...
[](#m-05-anyone-can-create-disputes-if-contractor-is-not-set)[\[M-05\] Anyone can create disputes if `contractor` is not set](https://github.com/code-423n4/2022-08-rigor-findings/issues/327)
----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

_Submitted by berndartmueller, also found by 0xA5DF, arcoun, rotcivegaf, and wastewa_

[Project.sol#L498-L502](https://github.com/code-423n4/2022-08-rigor/blob/5ab7ea84a1516cb726421ef690af5bc41029f88f/contracts/Project.sol#L498-L502)  
...
[](#m-06-attacker-can-drain-all-the-projects-within-minutes-if-admin-account-has-been-exposed)[\[M-06\] Attacker can drain all the projects within minutes, if admin account has been exposed](https://github.com/code-423n4/2022-08-rigor-findings/issues/264)
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

_Submitted by 0xA5DF, also found by Lambda and sseefried_

[HomeFi.sol#L156-L169](https://github.com/code-423n4/2022-08-rigor/blob/5ab7ea84a1516cb726421ef690af5bc41029f88f/contracts/HomeFi.sol#L156-L169)  
[HomeFi.sol#L199-L208](https://github.com/code-423n4/2022-08-rigor/blob/5ab7ea84a1516cb726421ef690af5bc41029f88f/contracts/HomeFi.sol#L199-L208)  
...
[](#m-07-projectraisedispute-doesnt-use-approvedhashes---meaning-users-who-use-contracts-cant-raise-disputes)[\[M-07\] `Project.raiseDispute()` doesn’t use approvedHashes - meaning users who use contracts can’t raise disputes](https://github.com/code-423n4/2022-08-rigor-findings/issues/340)
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

_Submitted by 0xA5DF_

[Project.sol#L493-L536](https://github.com/code-423n4/2022-08-rigor/blob/5ab7ea84a1516cb726421ef690af5bc41029f88f/contracts/Project.sol#L493-L536)  
...
[](#m-08-builders-must-pay-more-interest-when-the-system-is-paused)[\[M-08\] Builders must pay more interest when the system is paused.](https://github.com/code-423n4/2022-08-rigor-findings/issues/248)
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

_Submitted by hansfriese, also found by 0x52, 0xNazgul, and rbserver_

[Community.sol#L455](https://github.com/code-423n4/2022-08-rigor/blob/5ab7ea84a1516cb726421ef690af5bc41029f88f/contracts/Community.sol#L455)  
[Community.sol#L484](https://github.com/code-423n4/2022-08-rigor/blob/5ab7ea84a1516cb726421ef690af5bc41029f88f/contracts/Community.sol#L484)  
[Community.sol#L509](https://github.com/code-423n4/2022-08-rigor/blob/5ab7ea84a1516cb726421ef690af5bc41029f88f/contracts/Community.sol#L509)  
...
[](#m-09-it-should-not-submit-a-project-with-no-total-budget-requires-at-least-one-task-with-cost--0)[\[M-09\] It should not submit a project with no total budget. Requires at least one task with cost > 0](https://github.com/code-423n4/2022-08-rigor-findings/issues/348)
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

_Submitted by cryptonue, also found by aez121, hansfriese, obront, rbserver, and saneryee_

[Community.sol#L206-L282](https://github.com/code-423n4/2022-08-rigor/blob/main/contracts/Community.sol#L206-L282)  
...
> _“Note that you cannot submit a project with no total budget. Therefore it requires at least one task with a budget > 0.”_
...
[](#m-10-possible-dos-in-lendtoproject-and-togglelendingneeded-function-because-unbounded-loop-can-run-out-of-gas)[\[M-10\] Possible DOS in `lendToProject()` and `toggleLendingNeeded()` function because unbounded loop can run out of gas](https://github.com/code-423n4/2022-08-rigor-findings/issues/336)
--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

_Submitted by minhquanym, also found by berndartmueller, Chom, and scaraven_

[Project.sol#L710](https://github.com/code-423n4/2022-08-rigor/blob/5ab7ea84a1516cb726421ef690af5bc41029f88f/contracts/Project.sol#L710)  
...
[](#m-11-owner-of-project-nft-has-no-purpose)[\[M-11\] Owner of project NFT has no purpose](https://github.com/code-423n4/2022-08-rigor-findings/issues/413)
------------------------------------------------------------------------------------------------------------------------------------------------------------
...
[HomeFi.sol#L225](https://github.com/code-423n4/2022-08-rigor/blob/5ab7ea84a1516cb726421ef690af5bc41029f88f/contracts/HomeFi.sol#L225)  
...
[](#m-12-updateprojecthash-does-not-check-project-address)[\[M-12\] `updateProjectHash` does not check project address](https://github.com/code-423n4/2022-08-rigor-findings/issues/347)
----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

_Submitted by MEP, also found by byndooa, Haipls, and minhquanym_

[Project.sol#L162](https://github.com/code-423n4/2022-08-rigor/blob/5ab7ea84a1516cb726421ef690af5bc41029f88f/contracts/Project.sol#L162)  
...
[](#m-13-in-projectsetcomplete-the-signature-can-be-reused-when-the-first-call-is-reverted-for-some-reason)[\[M-13\] In `Project.setComplete()`, the signature can be reused when the first call is reverted for some reason](https://github.com/code-423n4/2022-08-rigor-findings/issues/263)
----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

_Submitted by hansfriese, also found by cccz_

[Project.sol#L330](https://github.com/code-423n4/2022-08-rigor/blob/5ab7ea84a1516cb726421ef690af5bc41029f88f/contracts/Project.sol#L330)  
...
[](#m-14-incorrect-initialization-of-smart-contracts-with-access-control-issue)[\[M-14\] Incorrect initialization of smart contracts with Access Control issue](https://github.com/code-423n4/2022-08-rigor-findings/issues/6)
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

_Submitted by Haipls, also found by byndooa, cryptphi, and TrungOre_

[HomeFiProxy.sol#L216-L230](https://github.com/code-423n4/2022-08-rigor/blob/5ab7ea84a1516cb726421ef690af5bc41029f88f/contracts/HomeFiProxy.sol#L216-L230)  
[Community.sol#L102-L119](https://github.com/code-423n4/2022-08-rigor/blob/5ab7ea84a1516cb726421ef690af5bc41029f88f/contracts/Community.sol#L102-L119)  
[DebtToken.sol#L43-L58](https://github.com/code-423n4/2022-08-rigor/blob/5ab7ea84a1516cb726421ef690af5bc41029f88f/contracts/DebtToken.sol#L43-L58)  
[Disputes.sol#L74-L81](https://github.com/code-423n4/2022-08-rigor/blob/5ab7ea84a1516cb726421ef690af5bc41029f88f/contracts/Disputes.sol#L74-L81)  
[HomeFi.sol#L92-L120](https://github.com/code-423n4/2022-08-rigor/blob/5ab7ea84a1516cb726421ef690af5bc41029f88f/contracts/HomeFi.sol#L92-L120)  
[Project.sol#L94-L105](https://github.com/code-423n4/2022-08-rigor/blob/5ab7ea84a1516cb726421ef690af5bc41029f88f/contracts/Project.sol#L94-L105)  
[ProjectFactory.sol#L45-L55](https://github.com/code-423n4/2022-08-rigor/blob/5ab7ea84a1516cb726421ef690af5bc41029f88f/contracts/ProjectFactory.sol#L45-L55)  
...
### [](#examples-of-exploiting-the-vulnerability)Examples of exploiting the vulnerability
...
[](#m-15-projectaddtasks-wouldnt-work-properly-when-its-called-from-disputes-contract)[\[M-15\] `Project.addTasks()` wouldn’t work properly when it’s called from disputes contract.](https://github.com/code-423n4/2022-08-rigor-findings/issues/233)
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

_Submitted by hansfriese, also found by Lambda_

[Project.sol#L238](https://github.com/code-423n4/2022-08-rigor/blob/5ab7ea84a1516cb726421ef690af5bc41029f88f/contracts/Project.sol#L238)  
...
[](#m-16-new-subcontractor-can-be-set-for-a-scconfirmed-task-without-current-subcontractor-consent)[\[M-16\] New subcontractor can be set for a SCConfirmed task without current subcontractor consent](https://github.com/code-423n4/2022-08-rigor-findings/issues/378)
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
[Project.sol#L295-L316](https://github.com/code-423n4/2022-08-rigor/blob/5ab7ea84a1516cb726421ef690af5bc41029f88f/contracts/Project.sol#L295-L316)  
...
[Project.sol#L747-L762](https://github.com/code-423n4/2022-08-rigor/blob/5ab7ea84a1516cb726421ef690af5bc41029f88f/contracts/Project.sol#L747-L762)  
...
[Tasks.sol#L106-L111](https://github.com/code-423n4/2022-08-rigor/blob/5ab7ea84a1516cb726421ef690af5bc41029f88f/contracts/libraries/Tasks.sol#L106-L111)  
...
[Project.sol#L349-L356](https://github.com/code-423n4/2022-08-rigor/blob/5ab7ea84a1516cb726421ef690af5bc41029f88f/contracts/Project.sol#L349-L356)  
...
[Project.sol#L310-L313](https://github.com/code-423n4/2022-08-rigor/blob/5ab7ea84a1516cb726421ef690af5bc41029f88f/contracts/Project.sol#L310-L313)  
...
[Project.sol#L386-L403](https://github.com/code-423n4/2022-08-rigor/blob/5ab7ea84a1516cb726421ef690af5bc41029f88f/contracts/Project.sol#L386-L403)  
...
[Project.sol#L477-L481](https://github.com/code-423n4/2022-08-rigor/blob/5ab7ea84a1516cb726421ef690af5bc41029f88f/contracts/Project.sol#L477-L481)  
...
[Project.sol#L855-L861](https://github.com/code-423n4/2022-08-rigor/blob/5ab7ea84a1516cb726421ef690af5bc41029f88f/contracts/Project.sol#L855-L861)  
...
> When a SC accepts an invitation the task is marked as active [Tasks.sol#L128](https://github.com/code-423n4/2022-08-rigor/blob/5ab7ea84a1516cb726421ef690af5bc41029f88f/contracts/libraries/Tasks.sol#L128).  
...
> 
> [Project.sol#L422-L461](https://github.com/code-423n4/2022-08-rigor/blob/5ab7ea84a1516cb726421ef690af5bc41029f88f/contracts/Project.sol#L422-L461)  
> 
...
> 
> [Project.sol#L391-L403](https://github.com/code-423n4/2022-08-rigor/blob/5ab7ea84a1516cb726421ef690af5bc41029f88f/contracts/Project.sol#L391-L403)  
> 
>             // Decode params from _data
>             (
>                 uint256 _taskID,
...
> 
> [Tasks.sol#L160-L164](https://github.com/code-423n4/2022-08-rigor/blob/5ab7ea84a1516cb726421ef690af5bc41029f88f/contracts/libraries/Tasks.sol#L160-L164)  
> 
...
>         }
> 
> [Tasks.sol#L119-L129](https://github.com/code-423n4/2022-08-rigor/blob/5ab7ea84a1516cb726421ef690af5bc41029f88f/contracts/libraries/Tasks.sol#L119-L129)  
> 
...
> 
> [Tasks.sol#L42-L46](https://github.com/code-423n4/2022-08-rigor/blob/5ab7ea84a1516cb726421ef690af5bc41029f88f/contracts/libraries/Tasks.sol#L42-L46)  
> 
...
> 
> [Project.sol#L443-L460](https://github.com/code-423n4/2022-08-rigor/blob/5ab7ea84a1516cb726421ef690af5bc41029f88f/contracts/Project.sol#L443-L460)  
> 
>                     else {
>     -                  // Un-confirm SC, mark task as inactive, mark allocated as false, mark lifecycle as None
>     
...
[](#m-17-malicious-delegated-contractor-can-block-funding-tasks-or-mark-tasks-as-complete)[\[M-17\] Malicious delegated contractor can block funding tasks or mark tasks as complete](https://github.com/code-423n4/2022-08-rigor-findings/issues/320)
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

_Submitted by indijanc_

[Project.sol#L219](https://github.com/code-423n4/2022-08-rigor/blob/main/contracts/Project.sol#L219)  
[Project.sol#L655](https://github.com/code-423n4/2022-08-rigor/blob/main/contracts/Project.sol#L655)  
...
[](#m-18-task-functionality-completely-sidestepped-via-autowithdraw)[\[M-18\] Task Functionality completely sidestepped via `autoWithdraw`](https://github.com/code-423n4/2022-08-rigor-findings/issues/281)
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

_Submitted by GalloDaSballo_

[Project.sol#L770](https://github.com/code-423n4/2022-08-rigor/blob/5ab7ea84a1516cb726421ef690af5bc41029f88f/contracts/Project.sol#L770)  
...
[](#m-19-changeorder-requires-subcontractor-signature-when-the-subcontractor-address-is-0)[\[M-19\] `changeOrder` requires subcontractor signature when the subcontractor address is 0](https://github.com/code-423n4/2022-08-rigor-findings/issues/85)
-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

_Submitted by Lambda_

[Project.sol#L402](https://github.com/code-423n4/2022-08-rigor/blob/f2498c86dbd0e265f82ec76d9ec576442e896a87/contracts/Project.sol#L402)  
[Project.sol#L485](https://github.com/code-423n4/2022-08-rigor/blob/f2498c86dbd0e265f82ec76d9ec576442e896a87/contracts/Project.sol#L485)  
...
[](#m-20-projectsol-and-communitysol-have-no-way-to-revoke-a-hash-in-approvedhashes)[\[M-20\] `Project.sol` and `Community.sol` have no way to revoke a hash in approvedHashes](https://github.com/code-423n4/2022-08-rigor-findings/issues/64)
-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

_Submitted by 0x52_

[Community.sol#L501-L506](https://github.com/code-423n4/2022-08-rigor/blob/5ab7ea84a1516cb726421ef690af5bc41029f88f/contracts/Community.sol#L501-L506)  
[Project.sol#L108-L115](https://github.com/code-423n4/2022-08-rigor/blob/5ab7ea84a1516cb726421ef690af5bc41029f88f/contracts/Project.sol#L108-L115)  
...
### [](#homefisolcreateproject-projectcount-should-be-cachedsaves--71-gas)HomeFi.sol.createProject(): projectCount should be cached(Saves ~ 71 gas)
...
SLOAD 1 [Line 228](https://github.com/code-423n4/2022-08-rigor/blob/b17b2a11d04289f9e927c71703b42771dd7b86a4/contracts/HomeFi.sol#L228)  
SLOAD 2 [Line 229](https://github.com/code-423n4/2022-08-rigor/blob/b17b2a11d04289f9e927c71703b42771dd7b86a4/contracts/HomeFi.sol#L229)  
SLOAD 3 [Line 231](https://github.com/code-423n4/2022-08-rigor/blob/b17b2a11d04289f9e927c71703b42771dd7b86a4/contracts/HomeFi.sol#L231)  
...
### [](#homefisolmintnft-projectcount-should-be-cached)HomeFi.sol.mintNFT(): projectCount should be cached
...
### [](#projectsolupdateprojecthash-hashchangenonce-should-be-cached-saves--101-gas)Project.sol.updateProjectHash(): hashChangeNonce should be cached (saves ~ 101 gas)
...
SLOAD 1: [Line 176](https://github.com/code-423n4/2022-08-rigor/blob/b17b2a11d04289f9e927c71703b42771dd7b86a4/contracts/Project.sol#L176)  
SLOAD 2: [Line 179](https://github.com/code-423n4/2022-08-rigor/blob/b17b2a11d04289f9e927c71703b42771dd7b86a4/contracts/Project.sol#L179)  
...
### [](#projectsolupdatetaskhash-hashchangenonce-should-be-cachedsaves--98-gas)Project.sol.updateTaskHash(): hashChangeNonce should be cached(saves ~ 98 gas)
...
In the above function, there are two SLOADS that can be replaced with a cached variable.  
SLOAD 1: [Line 277](https://github.com/code-423n4/2022-08-rigor/blob/b17b2a11d04289f9e927c71703b42771dd7b86a4/contracts/Project.sol#L277)  
SLOAD 2: [Line 290](https://github.com/code-423n4/2022-08-rigor/blob/b17b2a11d04289f9e927c71703b42771dd7b86a4/contracts/Project.sol#L290)  
...
### [](#projectsolallocatefunds_changeorderedtasklength-should-be-cachedsaves--118-gas)Project.sol.allocateFunds():\_changeOrderedTask.length should be cached(Saves ~ 118 gas)
...
SLOAD 1: [Line 592](https://github.com/code-423n4/2022-08-rigor/blob/b17b2a11d04289f9e927c71703b42771dd7b86a4/contracts/Project.sol#L592)  
SLOAD 2: [Line 610](https://github.com/code-423n4/2022-08-rigor/blob/b17b2a11d04289f9e927c71703b42771dd7b86a4/contracts/Project.sol#L601)  
SLOAD 3: Read inside a for loop [Line 603](https://github.com/code-423n4/2022-08-rigor/blob/b17b2a11d04289f9e927c71703b42771dd7b86a4/contracts/Project.sol#L603)  
SLOAD 4: [Line 635](https://github.com/code-423n4/2022-08-rigor/blob/b17b2a11d04289f9e927c71703b42771dd7b86a4/contracts/Project.sol#L635)  
...
### [](#communitysolcreatecommunitycommunitycount-should-be-cached-saves-186-gas)Community.sol.createCommunity():communityCount should be cached( Saves ~186 gas)
...
SLOAD 1: [Line 140](https://github.com/code-423n4/2022-08-rigor/blob/b17b2a11d04289f9e927c71703b42771dd7b86a4/contracts/Community.sol#L140)  
SLOAD 2: [Line 143](https://github.com/code-423n4/2022-08-rigor/blob/b17b2a11d04289f9e927c71703b42771dd7b86a4/contracts/Community.sol#L143)  
SLOAD 3: [Line 150](https://github.com/code-423n4/2022-08-rigor/blob/b17b2a11d04289f9e927c71703b42771dd7b86a4/contracts/Community.sol#L150)  
...
### [](#projectsolallocatefunds-_changeorderedtasklength-should-be-cached---_changeorderedtask-is-a-storage-array)Project.sol.allocateFunds(): \_changeOrderedTask.length should be cached - \_changeOrderedTask is a storage array
...
[](#g-04-x-is-more-efficient-than-xsaves-6-gas)\[G-04\] ++x is more efficient than x++(Saves ~6 gas)
----------------------------------------------------------------------------------------------------

File: Community.sol [Line 140](https://github.com/code-423n4/2022-08-rigor/blob/b17b2a11d04289f9e927c71703b42771dd7b86a4/contracts/Community.sol#L140)  
...
[](#g-07-emitting-storage-values-instead-of-the-memory-onesaves-101-gas)\[G-07\] Emitting storage values instead of the memory one(saves ~101 gas)
--------------------------------------------------------------------------------------------------------------------------------------------------

Here, the values emitted shouldn’t be read from storage. The existing memory values should be used instead:

File: Project.sol [Line 144](https://github.com/code-423n4/2022-08-rigor/blob/b17b2a11d04289f9e927c71703b42771dd7b86a4/contracts/Project.sol#L144) average gas while using the storage value - 69561 average gas while using the memory value - 69460
...
The above operation cannot underflow due to the check on [Line 425](https://github.com/code-423n4/2022-08-rigor/blob/b17b2a11d04289f9e927c71703b42771dd7b86a4/contracts/Project.sol#L425) which ensures that `_taskCost` is greater than `_newCost` before the subtraction operation is performed.  
...
_Note: see warden’s [original submission](https://github.com/code-423n4/2022-08-rigor-findings/issues/391) for full list of instances._
...
### [](#projectsolupdateprojecthash---saves-19-gas)Project.sol.updateProjectHash() - (Saves ~19 gas)
...
### [](#projectsolupdatetaskhash---saves-19-gas)Project.sol.updateTaskHash() - (Saves ~19 gas)
...
### [](#homefisolmintnft)HomeFi.sol.mintNFT()
